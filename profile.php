...
function displayProfile($screen_name='') {
	...
	if ($screen_name == '') {
		$message_created_by = "$screen_name's Profile";
	} else {
		if (isset ( $_SESSION ['profile_image_url'] )) {
			$message_created_by = ""; //redundant, was used to concatenate profile_image_url
		}
		$message_created_by = " <span style='float:left'>&nbsp;&nbsp;<a href=$hostname/profile.php?screen_name=$screen_name>$screen_name's</a> profile (Total: $msg_cnt)</span>"; 
		...
		<input class='button_del' type='submit' value='Search profile' name='submit'  />
	...
	if ($msg_cnt > 0) {
		$message .= "<form method='post' action='/profile.php'>";
	...
	<th class='p5_th'><a href='$hostname/profile.php?order=prefix&message_from=0&more&asc_desc=$asc_desc&toggle&screen_name=$screen_name&search=$search'>
	Message</a></th>
	<th class='p5_th'><a href='$hostname/profile.php?order=created&message_from=0&more&asc_desc=$asc_desc&toggle&screen_name=$screen_name&search=$search'>
	Created</a></th>
	<th class='p5_th'><a href='$hostname/profile.php?order=last_viewed&message_from=0&more&asc_desc=$asc_desc&toggle&screen_name=$screen_name&search=$search'>
	Last Viewed</a></th>
	<th class='p5_th'><a href='$hostname/profile.php?order=views&message_from=0&more&asc_desc=$asc_desc&toggle&screen_name=$screen_name&search=$search'>
	Views</a></th>
	</tr>
	";
		...
		$more = "<a href='$hostname/profile.php?message_from=$message_from_more&more&order=$order&asc_desc=$asc_desc&screen_name=$screen_name&search=$search'
	                    >Next ></a>";
		$less = "<a href='$hostname/profile.php?message_from=$message_from_less&less&order=$order&asc_desc=$asc_desc&screen_name=$screen_name&search=$search'
	                   >&lt; Previous</a>";
